...
class ImageCLIP(nn.Module):
    ...
    def forward(self, src_input):
        ...
        x = self.model(src_input['imgs_id'].cuda(), src_input['kps_id'].cuda(),src_input['src_length_batch']) # [b, n, c]
...
class gloss_free_model(nn.Module):
    ...
    def share_forward(self, input):
        feature = self.backbone(input['imgs_id'].cuda(), input['kps_id'].cuda(),input['src_length_batch'])
        attention_mask = input['attention_mask']
        inputs_embeds = self.sign_emb(feature)
        inputs_embeds = self.embed_scale * inputs_embeds
        return inputs_embeds, attention_mask
    def forward(self, input, target):
        inputs_embeds, attention_mask = self.share_forward(input)
        '根据不同的文本模型，这里可能要调整'
        out = self.text_model(
